...
import { z } from "zod";
import { getNftBalances } from "../../tools/account/get_nft_balances";
import {
  GetNftBalancesInput,
 ...
 */
const getNftBalancesAction: Action = {
  ...
  handler: async (agent, input) => {
    try {
      const parsedInput = input as GetNftBalancesInput;
      const result: GetNftBalancesResponse = await getNftBalances(agent, parsedInput);
...
export default getNftBalancesAction;
